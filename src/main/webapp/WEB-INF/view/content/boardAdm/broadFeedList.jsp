...
<script type="text/javascript">
  ...
  $(function() {
    $(document).on('click', '#tb_broadFeedList img.thumbnail', function() {
      alert('1');
    });
  });
</script>
...
<script id="tmp_broadFeedTable" type="text/x-handlebars-template">
    ...
    <tr {{#dalbit_if inner '==' 1}} style="background-color : #dae3f3" {{/dalbit_if}}>
        ...
        <td class="word-break" style="padding: 5px; width: 400px;"><span class="pull-left">{{{replaceHtml feed_conts}}}</span></td>
        <td>
            {{#equal image_path ''}}
            {{else}}
            <img class="thumbnail" alt="your image" src="{{renderImage ../image_path}}" style='height:68px; width:68px; margin: auto; cursor: pointer' />
            {{/equal}}
        </td>
        <td>
            {{addComma tail_cnt}}
