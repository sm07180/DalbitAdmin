...
<script>
    ...
    var fnc_eventDetail = {};
//=------------------------------ Init / Event--------------------------------------------
    fnc_eventDetail.targetId= "eventDetail";

    fnc_eventDetail.init= function() {
        fnc_eventDetail.target = $("#"+fnc_eventDetail.targetId);

        if(common.isEmpty(getSelectDataInfo())){
            fnc_eventDetail.insertEventDetail();
        }else{
            console.log(getSelectDataInfo());
            fnc_eventDetail.dataKey = getSelectDataInfo().dataKey;
            fnc_eventDetail.data = getSelectDataInfo().data;

            fnc_eventDetail.updateEventDetail(fnc_eventDetail.data);
        }
    ...
    // 초기 설정
    fnc_eventDetail.initDetail= function() {
    ...
    // 이벤트 적용
    fnc_eventDetail.initEventDetail= function(){
        ...
        // 수정 버튼
        fnc_eventDetail.target.find("#updateBtn").on("click", function () {
            var data = fnc_eventDetail.getEventDetailData();
            if(!fnc_eventDetail.isValid(data)) {
                return false;
            }
...
//=------------------------------ Option --------------------------------------------
    // 등록 화면
    fnc_eventDetail.insertEventDetail= function() {
        var template = $('#tmp_eventDetailFrm').html();
        var templateScript = Handlebars.compile(template);
        fnc_eventDetail.target.find("#eventForm").html(templateScript);

        fnc_eventDetail.initDetail();
        fnc_eventDetail.initEventDetail();

        var scrollPosition = $("#tab_"+fnc_eventDetail.targetId).offset();
        util.scrollPostion(scrollPosition.top);
    };


    // 수정 화면
    fnc_eventDetail.updateEventDetail= function(data){
        dalbitLog(data);
        // form 띄우기
        ...
        // 캘린더
        fnc_eventDetail.target.find("#startDate").val(sDate.format('YYYY.MM.DD'));
        fnc_eventDetail.target.find("#endDate").val(eDate.format('YYYY.MM.DD'));
        fnc_eventDetail.target.find("#view-startDate").val(sViewDate.format('YYYY.MM.DD'));
        fnc_eventDetail.target.find("#view-endDate").val(eViewDate.format('YYYY.MM.DD'));

        // 시간 Select CSS 적용
        fnc_eventDetail.target.find("#event-div-startDate").find("#timeHour").val(common.lpad(sDate.hour() ,2,"0"));
        fnc_eventDetail.target.find("#event-div-startDate").find("#timeMinute").val(common.lpad(sDate.minute() ,2,"0"));
        fnc_eventDetail.target.find("#event-div-endDate").find("#timeHour").val(common.lpad(eDate.hour() ,2,"0"));
        fnc_eventDetail.target.find("#event-div-endDate").find("#timeMinute").val(common.lpad(eDate.minute() ,2,"0"));
        fnc_eventDetail.target.find("#view-event-div-startDate").find("#timeHour").val(common.lpad(sViewDate.hour() ,2,"0"));
        fnc_eventDetail.target.find("#view-event-div-startDate").find("#timeMinute").val(common.lpad(sViewDate.minute() ,2,"0"));
        fnc_eventDetail.target.find("#view-event-div-endDate").find("#timeHour").val(common.lpad(eViewDate.hour() ,2,"0"));
        fnc_eventDetail.target.find("#view-event-div-endDate").find("#timeMinute").val(common.lpad(eViewDate.minute() ,2,"0"));
...
//=------------------------------ Data Handler ----------------------------------
    // 데이터 세팅
    fnc_eventDetail.getEventDetailData= function(){
        var resultJson ={};

        var formArray = fnc_eventDetail.target.find("#eventForm").serializeArray();
        for (var i = 0; i < formArray.length; i++){
            resultJson[formArray[i]['name']] = formArray[i]['value'];
        }

        //Date 처리이이이~~~
        var periodStartDiv = fnc_eventDetail.target.find("#event-div-startDate");
        ...
        var periodEndDiv = fnc_eventDetail.target.find("#event-div-endDate");
        ...
        var view_periodStartDiv = fnc_eventDetail.target.find("#view-event-div-startDate");
        ...
        var view_periodEndDiv = fnc_eventDetail.target.find("#view-event-div-endDate");
    ...
    fnc_eventDetail.isValid= function(data){
    ...
    // 등록 완료
    fnc_eventDetail.event_fn_success= function(dst_id, response){
    ...
    fnc_eventDetail.event_fn_fail= function(dst_id, response){
        console.log("에러!!!");
    };
