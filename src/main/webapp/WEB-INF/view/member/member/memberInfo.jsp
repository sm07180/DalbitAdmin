...
<script type="text/javascript">
  ...
  // 스톤 목록 페이징 정보
  let stonePagingInfo = new PAGING_INFO(0, 1, 20);
  function getStoneDetail(_memNo) {
    $("#member_detailFrm").removeClass('show');
    let data = {
      memNo: _memNo,
      pageNo: stonePagingInfo.pageNo,
      pagePerCnt: stonePagingInfo.pageCnt
    };
    let apiURL = '/rest/content/event/rebrand/collect-logs';
    util.getAjaxData("getCollectLogList", apiURL, data, function (id, response, params) {
      renderStoneDetail(id, response, params);
    }, null, {type: 'GET'});
  }
  ...
  function renderStoneDetail(id, response, params) {
    ...
    template = $('#tmp_stone_list').html();
    templateScript = Handlebars.compile(template);
    context = response.listData.map(function (item, index) {
      item.index_no = response.totalCnt - (((stonePagingInfo.pageNo - 1) * stonePagingInfo.pageCnt) + index);
      item.memo = item.dalla_gubun +  ' ' + stoneType[item.chrgr_slct] + item.ins_dalla_cnt + ' / ' + stoneAdminType[item.chrgr_yn];
      return item;
    });
    html = templateScript(context);
    $("#member_detailFrm").html(html);
    $("#member_detailFrm").addClass('show');

    stonePagingInfo.totalCnt = response.totalCnt;
    util.renderPagingNavigation('stone-list-top', stonePagingInfo);
    util.renderPagingNavigation('stone-list-bottom', stonePagingInfo);

    if (response.listData.length === 0) {
      $('#stone-list-top').hide();
      $('#stone-list-bottom').hide();
    } else {
      $('#stone-list-top').show();
      $('#stone-list-bottom').show();
    }

    var scrollPosition = $("#member_detailFrm").offset();
  ...
  function init() {
    ...
    // 달라스톤 등록
    $('.btn-ins-stone').on('click', function () {
      var $this = $(this);
      ...
      var $insDallaSlct = $('#insDallaSlct-' + stone);
      var $insDallaCnt = $('#insDalla-' + stone);

      let data = {
        memNo: memNo,
        dallaGubun: stone,
        insDallaCnt: $insDallaCnt.val(),
        chrgrMemName: '',
        insDallaSlct: $insDallaSlct.val()
      };
      let apiURL = '/rest/content/event/rebrand/collects';
      util.getAjaxData("createCollect", apiURL, data, function (id, response, params) {
    ...
    // 달라스톤 상세
    $('#btn-stone-detail').on('click', function () {
      var $this = $(this);
      var memNo = $this.data('memno');
      if (!memNo) {
        return;
    ...
    <table class="table table-bordered table-dalbit borderBlack" style="margin-bottom: 0px;">
        ...
        <tr>
            <th rowspan="5">
                ...
                <br>
                <button type="button" id="bt_img" class="btn btn-default btn-sm no-margin" style="margin-left: 10px"
                        data-memno="{{mem_no}}" data-nickname="{{nickName}}">초기화
                </button>
                <br>
                <button type="button" id="bt_profileImg_editHistory" class="btn btn-default btn-sm mt5">상세</button>
                {{/equal}}
            </th>
            <td rowspan="5">
                <div class="col-md-8 no-padding">
                    <form id="profileImg" method="post" enctype="multipart/form-data">
                        <img id="image_section" class="thumbnail fullSize_background no-padding"
                             src="{{renderProfileImage profileImage memSex}}" alt="your image"
            ...
            <th rowspan="5">
                ...
                {{#equal memWithdrawal '0'}}
                <br>
                <button type="button" id="bt_bg_img" class="btn btn-default btn-sm no-margin" style="margin-left: 10px"
                        data-memno="{{mem_no}}" data-nickname="{{nickName}}">초기화
                </button>
                <br>
                <button type="button" id="bt_bgImg_editHistory" class="btn btn-default btn-sm mt5">상세</button>
                {{/equal}}
            </th>
            <td rowspan="5">
                <form id="bgImg" method="post" enctype="multipart/form-data">
                    <img id="image_bg_section" class="thumbnail fullSize_background no-padding"
                         src="{{renderProfileImage roomBgImage memSex}}" alt="your image"
                         style="width: 150px;height: 150px"/>
                </form>
            </td>
        </tr>
        <tr>
            ...
            <th>경험치</th>
            <td style="text-align: left;border-right-color:white;border-right-width:0px;">{{addComma exp}}({{substr
                expPer '0' '5'}}%)
            </td>
            <td>
        ...
        <tr>
            ...
            <td style="text-align: left">
                ...
                임시정지
                {{else}}
                <span {{#dalbit_if memState '==' 7}} style="color:blue" {{/dalbit_if}}>{{{getCommonCodeLabel memState
                'mem_state'}}}</span>
                {{/dalbit_if}}
                {{else}}
                <span {{#dalbit_if memState '==' 7}} style="color:blue" {{/dalbit_if}}>{{{getCommonCodeLabel memState
        ...
        <tr>
            ...
            <td rowspan="2" colspan="3" style="text-align: left" id="memberProfileMsg">
                ...
                <button type="button" id="bt_profileMsg_editHistory" class="btn btn-default btn-sm pull-right ml5">상세
                </button>
                <button type="button" id="bt_profileMsg_del" class="btn btn-default btn-sm pull-right "
                        style="background-color: #46B0CF; border-color: #46B0CF">삭제
                </button>
            </td>
            <th>방송상태</th>
            <td colspan="3"
                style="text-align: left;{{#dalbit_if type_media '==' 'v'}} background-color: #fbe5d6 {{/dalbit_if}} ">
                {{{icon_broadcastState}}}
                {{#equal broadcastState 'ON'}}
                - 방송제목 : {{{roomNoLink ../title ../room_no}}}
                {{/equal}}
                {{#dalbit_if type_media '==' 'a'}}
                <img src="https://image.dalbitlive.com/svg/ico_live_audio.svg" alt="your image"
                     style="width: 33px;height: 33px"/>
                {{/dalbit_if}}
                {{#dalbit_if type_media '==' 'v'}}
                <img src="https://image.dalbitlive.com/svg/ico_live_video_b.svg" alt="your image"
                     style="width: 33px;height: 33px"/>
                {{/dalbit_if}}
            </td>
            <th>마이크</th>
            ...
            <td>
                ...
                {{/equal}}
                {{#equal ../hide 1}}
                <button type="button" id="bt_broadCastHideCancel" class="btn btn-info btn-sm pull-right">방송방 숨김 해제
                </button>
                {{/equal}}
                {{/equal}}
            </td>
        ...
        <tr>
            ...
            <td colspan="6" style="text-align: left;border-right-color:white;border-right-width:0px;">
                ...
                <button type="button" id="bt_listenExit"
                        ...
                        data-listen_room_no="{{../listen_room_no}}">청취종료
                </button>
                {{/equal}}
            </td>
            <td>
                <button type="button" id="bt_forcedExit" class="btn btn-danger btn-sm pull-right"
                        onclick="forcedListenExit();">청취강제종료
                </button>
            </td>
        </tr>
        <tr>
            ...
            <th>클립 청취</th>
            <td colspan="3"><a href="javascript: $('#tab_clipListenList').click();">{{addComma clipPlayCnt}} 건 (
                {{addComma clipListenCnt}} 건)</a></td>
        </tr>
        <tr>
            <th>닉네임</th>
            <td style="text-align: left;">
                {{nickName}}
                {{#equal memWithdrawal '0'}}
                <button type="button" id="bt_resatNick" class="btn btn-default btn-sm pull-right"
                        data-memno="{{mem_no}}" data-nickname="{{nickName}}" data-userId="{{userId}}">초기화
                </button>
                {{/equal}}
            </td>
            <%--<td colspan="2">--%>
        ...
        <tr>
            ...
            <td>
                ...
                {{#equal memWithdrawal '0'}}
                <button type="button" id="bt_userName" class="btn btn-default btn-sm" data-memno="{{mem_no}}"
            ...
            <td style="text-align: left">
                ...
                {{{getBadge 'red' 'red' '' '스타DJ' '100px' '20px' 10 '15px' '15px'}}}
                {{/dalbit_if}}
            ...
            <td colspan="6" style="text-align: left">
                ...
                <c:if test="${insertYn eq 'Y'}">
                    ...
                    <span class="col-md-6 no-padding pull-right" id="sp_byeolPointEdit">
                            ...
                            <button type="button" id="bt_byeolAdd" class="btn btn-default btn-sm"
                                    data-memno="{{mem_no}}">변경</button>
                        </span>
                    {{/equal}}
                </c:if>
            </td>
        ...
        <tr>
            ...
            <td style="text-align: left;border-right-color:white;border-right-width:0px;">
                <div id="div_socialId">
                    <input type="text" class="form-control" id="txt_socialId" value="{{socialId}}"
                           onkeyup="fnChkByte(this);" style="width:75%;">
                    {{#equal memWithdrawal '0'}}
                    <button type="button" id="bt_socialId" class="btn btn-default btn-sm" data-memno="{{mem_no}}"
                            data-nickname="{{nickName}}">변경
                    </button>
                    {{/equal}}
                </div>
            ...
            <td colspan="3" style="text-align: left">
                ...
                <c:if test="${insertYn eq 'Y'}">
                    <span class="col-md-9 no-padding" id="">
                        ...
                        <button type="button" id="bt_boostAdd" class="btn btn-default btn-sm" data-memno="{{mem_no}}"
                                data-boostcnt="{{boostItemCnt}}">변경</button>
        ...
        <tr>
            ...
            <td style="text-align: left;border-right-color:white;border-right-width:0px;">
                ...
                {{#equal memWithdrawal '0'}}
                <button type="button" id="bt_phon" class="btn btn-default btn-sm" data-memno="{{mem_no}}"
                        data-nickname="{{nickName}}">변경
                </button>
                {{/equal}}
            </td>
        ...
        <tr>
            ...
            <td class="font-bold">
                {{#equal auth_yn 'Yes'}}
                {{../comm_company}} &nbsp&nbsp|&nbsp&nbsp <label
                    style="color: red; font-weight: bold">{{../auth_yn}}</label>
                <button type="button" id="bt_cancel_cert" class="btn btn-default btn-sm pull-right ml5"
                        style="background-color: #46B0CF; border-color: #46B0CF">철회
                </button>
                {{/equal}}
                {{#equal auth_yn 'No'}}
                <label style="font-weight: bold;">{{../auth_yn}}</label>
                {{#equal ../certificationBakYn 'Y'}}
                <button type="button" id="bt_rollback_cert" class="btn btn-default btn-sm pull-right ml5"
                        style="background-color: #46B0CF; border-color: #46B0CF">복귀
                </button>
                {{/equal}}
            ...
            <td>
                ...
                <button type="button" id="bt_recant" class="btn btn-default btn-sm pull-right ml5"
                        style="background-color: #46B0CF; border-color: #46B0CF">철회
                </button>
                ...
                <button type="button" id="bt_back_recant" class="btn btn-default btn-sm pull-right ml5"
                        style="background-color: #46B0CF; border-color: #46B0CF">복귀
                </button>
                <button type="button" id="bt_agree_info" class="btn btn-default btn-sm pull-right">동의정보</button>
                {{/equal}}
            ...
            <th>최근정보<br/>수정자</th>
            <td colspan="2" style="text-align: left;border-right-color:white;border-right-width:0px;">{{lastOpName}}
            </td>
            <td>
        ...
        <tr>
            ...
            <td style="text-align: left">
                ...
                {{#equal memWithdrawal '0'}}
                <button type="button" id="bt_birth" class="btn btn-default btn-sm" data-memno="{{../mem_no}}"
                        data-nickname="{{../nickName}}">변경
                </button>
                {{/equal}}
            </td>
        ...
        <tr>
            ...
            <td style="text-align: left">
                {{#equal memWithdrawal '0'}}
                <button type="button" id="bt_resatPass" class="btn btn-default btn-sm" data-memno="{{mem_no}}"
                        data-nickname="{{nickName}}">초기화
                </button>
                {{/equal}}
            </td>
            ...
            <th>최근메모<br>등록</th>
            <td colspan="2" style="text-align: left;border-right-color:white;border-right-width:0px;">등록: {{addComma
                opMemoCnt}} 건
            </td>
            <td>
            ...
            <td colspan="3" style="text-align: left;border-right-color:white;border-right-width:0px;">
                <span class="col-md-3 no-padding"
                      style="text-align: left; line-height: 30px;">{{addComma couponCnt}} 개</span>
                <c:if test="${insertYn eq 'Y'}">
                    <span class="col-md-9 no-padding" id="">
                        <select id="rouletteCouponPlusMinus" name="rouletteCouponPlusMinus"
                                class="form-control searchType">
                            <option value="1">+</option>
                            <option value="3">-</option>
                        </select>
                        <input type="text" class="form-control" id="txt_rouletteCouponAddCnt" style="width:50px"
                               maxlength="4">
                        <button type="button" id="bt_rouletteCouponAdd" class="btn btn-default btn-sm"
                                data-memno="{{mem_no}}" data-roulette-coupon-cnt="{{couponCnt}}">변경</button>
...
<script id="tmp_member_detailFrm" type="text/x-handlebars-template">
    <div class="widget-content mt5">
        <ul class="nav nav-tabs nav-tabs-custom-colored" role="tablist" id="tab_infoDetail">
            <li class="active" id="detail1"><a href="#memberInfoDetail" role="tab" data-toggle="tab"
                                               id="tab_memberInfoDetail"></a></li>
            <li class="hide" id="detail2"><a href="#memberInfoDetail2" role="tab" data-toggle="tab"
                                             id="tab_memberInfoDetail2"></a></li>
        </ul>
        <div class="tab-content" style="padding-top: 0px;">
            <div class="tab-pane fade in active" id="memberInfoDetail">
                <div class="widget-content mt5">
                    <ul class="nav nav-tabs nav-tabs-custom-colored" role="tablist" id="tab_adminMemo"
                        style="display: none">
                        <li class="active"><a href="#memoContent" role="tab" data-toggle="tab" id="tab_memoMember"
                                              onclick="adminMemoSubTabClick(1)">회원</a></li>
                        <li><a href="#memoContent" role="tab" data-toggle="tab" id="tab_memoBroadcast"
                               onclick="adminMemoSubTabClick(2)">방송방</a></li>
                        <li><a href="#memoContent" role="tab" data-toggle="tab" id="tab_memoCast"
                               onclick="adminMemoSubTabClick(3)">클립</a></li>
                    </ul>
                    <div class="tab-pane fade in active" id="memoContent">
                        <div class="widget widget-table">
                            <div class="widget-content">
                                <table id="info_detail"
    ...
    <div class="modal-dialog" style="max-width: 700px;min-width: 400px; width: auto; display: table;">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true" id="layerCloseBtn">&times;
                </button>
...
<script id="tmp_memberAccumData" type="text/x-handlebars-template">
    <table class="table table-sorting table-hover table-bordered borderBlack _tableHeight"
           style="width: 60%;margin-bottom: 5px;" data-height="23px">
        <tr>
...
<script id="tmp_memberAccumData2" type="text/x-handlebars-template">
    <table class="table table-sorting table-hover table-bordered borderBlack _tableHeight"
           style="width: 75%;margin-bottom: 5px;" data-height="23px">
        <tr>
...
<script id="tmp_stone_list" type="text/x-handlebars-template">
    <ul class="nav nav-tabs nav-tabs-custom-colored" role="tablist">
        <li class="active"><a href="javascript:void(0);" role="tab" data-toggle="tab">달라져스 스톤</a></li>
    </ul>
    <div class="dataTables_paginate paging_full_numbers" id="stone-list-top"></div>
    <table id="stone-list-table" class="table table-sorting table-hover table-bordered">
        <colgroup>
            <col width="60px"/>
            <col width="150px"/>
            <col width="auto"/>
        ...
        <tbody id="stone-list-table-body">
        {{#each this as |data|}}
        <tr>
            <td>{{index_no}}</td>
            <td>{{ins_date}}</td>
    ...
    <div class="dataTables_paginate paging_full_numbers" id="stone-list-bottom"></div>
</script>