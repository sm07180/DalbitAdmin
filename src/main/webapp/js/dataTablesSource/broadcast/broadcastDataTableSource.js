var BroadcastDataTableSource = {
  'liveList': {
    ...
    , 'columns': [
      ...
      {
        'title': '<lable style="color:red">청취자</lable>',
        'data': 'liveListener',
        'color': 'red',
        'width': '40px',
        'render': function (data, type, row, meta) {
  ...
  'endLiveList': {
    ...
    , 'columns': [
      ...
      {
        'title': '<lable style="color:red">누적청취</lable><br />(신규 팬)',
        'data': 'totalListener',
        'width': '60px',
  ...
  'voteHistory': {
    'url': '/rest/broadcast/vote/list'
    , 'columns': [
      {'title': '생성일시', 'data': 'ins_date', 'width': '100px'},
      {
        'title': '투표제목', 'data': 'vote_title', 'width': '100px', 'render': function (data, type, row) {
          return '<a href="javascript://" onclick="callItemList(\'' + row.vote_no + '\');">' + data + '</a>';
        }
      },
      {'title': '항목수', 'data': 'vote_item_cnt', 'width': '100px'},
      {'title': '참여인원', 'data': 'vote_mem_cnt', 'width': '100px'},
      {
        'title': '상태', 'data': 'vote_end_slct', 'width': '100px', 'render': function (row) {
          return common.voteType[row];
        }
      },
      {'title': '시작일시', 'data': 'start_date', 'width': '100px'},
      {'title': '종료일시', 'data': 'end_date', 'width': '100px'},
    ]
    , 'createdRow': function (row, data, dataIndex) {
      if (data.inner == 1) {    // 테스트계정 row 색상 표시
        $(row).addClass("bg-testMember");
