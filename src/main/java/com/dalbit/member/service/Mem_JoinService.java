...
public class Mem_JoinService {
    ...
    public Model getJoinListExcel(P_MemberJoinInputVo pMemberJoinInputVo, Model model) {
        ...
        String[] headers = null;
        headers = new String[]{"No", "회원가입일시", "가입플랫폼", "OS", "회원번호", "로그인ID", "UserID", "닉네임", "이름", "연락처", "IP", "광고유입여부", "방생성수", "청취수", "결제수"};
        int[] headerWidths = {3000, 6000, 6000, 6000, 6000, 6000, 6000, 6000, 6000, 6000, 6000, 6000, 6000, 6000, 6000};
        ...
        for(int i = 0; i < list.size(); i++){
            ...
            hm.put("mem13", DalbitUtil.isEmpty(list.get(i).getListenCnt())         ? "" : list.get(i).getListenCnt());
            hm.put("mem14", DalbitUtil.isEmpty(list.get(i).getPayCnt())            ? "" : list.get(i).getPayCnt());
