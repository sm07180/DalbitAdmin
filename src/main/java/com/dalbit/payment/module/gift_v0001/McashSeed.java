...
public class McashSeed {
    ...
    private int NoRoundKeys = 32;
    private static int SeedBlockSize;
    private int SeedBlockLen = 128;
    ...
    private static void EndianChange(int[] dws) {
        dws[0] = dws[0] >>> 24 | dws[0] << 24 | dws[0] << 8 & 16711680 | dws[0] >>> 8 & '\uff00';
    }

    private static int EndianChange(int dws) {
        return dws >>> 24 | dws << 24 | dws << 8 & 16711680 | dws >>> 8 & '\uff00';
    }
    ...
    private static void SeedRound(int[] L0, int[] L1, int[] R0, int[] R1, int[] K) {
        ...
        int T1 = R1[0] ^ K[1];
        T1 ^= T0;
        T00 = T0 < 0 ? (long)(T0 & 2147483647) | -2147483648L : (long)T0;
        T1 = SS0[GetB0(T1)] ^ SS1[GetB1(T1)] ^ SS2[GetB2(T1)] ^ SS3[GetB3(T1)];
        T11 = T1 < 0 ? (long)(T1 & 2147483647) | -2147483648L : (long)T1;
        T00 += T11;
    ...
    public static void SeedEncrypt(byte[] pbData, int[] pdwRoundKey, byte[] outData) {
        ...
        int nCount = 0;
        L0[0] = pbData[0] & 255;
        L0[0] = L0[0] << 8 ^ pbData[1] & 255;
        L0[0] = L0[0] << 8 ^ pbData[2] & 255;
        L0[0] = L0[0] << 8 ^ pbData[3] & 255;
        L1[0] = pbData[4] & 255;
        L1[0] = L1[0] << 8 ^ pbData[5] & 255;
        L1[0] = L1[0] << 8 ^ pbData[6] & 255;
        L1[0] = L1[0] << 8 ^ pbData[7] & 255;
        R0[0] = pbData[8] & 255;
        R0[0] = R0[0] << 8 ^ pbData[9] & 255;
        R0[0] = R0[0] << 8 ^ pbData[10] & 255;
        R0[0] = R0[0] << 8 ^ pbData[11] & 255;
        R1[0] = pbData[12] & 255;
        R1[0] = R1[0] << 8 ^ pbData[13] & 255;
        R1[0] = R1[0] << 8 ^ pbData[14] & 255;
        R1[0] = R1[0] << 8 ^ pbData[15] & 255;
        if (!ENDIAN) {
            EndianChange(L0);
            EndianChange(L1);
            EndianChange(R0);
        ...
        K[1] = pdwRoundKey[var10++];
        SeedRound(L0, L1, R0, R1, K);
        K[0] = pdwRoundKey[var10++];
        K[1] = pdwRoundKey[var10++];
        ...
        for(int i = 0; i < 4; ++i) {
            outData[i] = (byte)(R0[0] >>> 8 * (3 - i) & 255);
            outData[4 + i] = (byte)(R1[0] >>> 8 * (3 - i) & 255);
            outData[8 + i] = (byte)(L0[0] >>> 8 * (3 - i) & 255);
            outData[12 + i] = (byte)(L1[0] >>> 8 * (3 - i) & 255);
        }
    ...
    public static void SeedEncRoundKey(int[] pdwRoundKey, byte[] pbUserKey) {
        ...
        int nCount = 2;
        A[0] = pbUserKey[0] & 255;
        A[0] = A[0] << 8 ^ pbUserKey[1] & 255;
        A[0] = A[0] << 8 ^ pbUserKey[2] & 255;
        A[0] = A[0] << 8 ^ pbUserKey[3] & 255;
        B[0] = pbUserKey[4] & 255;
        B[0] = B[0] << 8 ^ pbUserKey[5] & 255;
        B[0] = B[0] << 8 ^ pbUserKey[6] & 255;
        B[0] = B[0] << 8 ^ pbUserKey[7] & 255;
        C[0] = pbUserKey[8] & 255;
        C[0] = C[0] << 8 ^ pbUserKey[9] & 255;
        C[0] = C[0] << 8 ^ pbUserKey[10] & 255;
        C[0] = C[0] << 8 ^ pbUserKey[11] & 255;
        D[0] = pbUserKey[12] & 255;
        D[0] = D[0] << 8 ^ pbUserKey[13] & 255;
        D[0] = D[0] << 8 ^ pbUserKey[14] & 255;
        D[0] = D[0] << 8 ^ pbUserKey[15] & 255;
        if (!ENDIAN) {
        ...
        pdwRoundKey[var10++] = K[1];
        EncRoundKeyUpdate1(K, A, B, C, D, 2);
        pdwRoundKey[var10++] = K[0];
        pdwRoundKey[var10++] = K[1];
    ...
    public static String encodeString(String inData, byte[] szKey) {
        String rtn = "";
        int[] sRoundKey = new int[32];
        SeedEncRoundKey(sRoundKey, szKey);
        byte[] sbuffer = inData.getBytes();
        int rt = (sbuffer.length + SeedBlockSize - 1) / SeedBlockSize;
        int pos1 = sbuffer.length / SeedBlockSize;
        int pos2 = sbuffer.length % SeedBlockSize;

        for(int j = 0; j < rt; ++j) {
            int pos = 0;
            byte[] sSource = new byte[BuffSize];
            byte[] sTarget = new byte[BuffSize];
            int end;
            int s;
            int i;
            if (pos1 - j != 0) {
                end = j * SeedBlockSize + SeedBlockSize;

                for(s = j * SeedBlockSize; s < end; ++pos) {
                    sSource[pos] = sbuffer[s];
                    ++s;
                }
            } else {
                end = j * SeedBlockSize + pos2;
                ...
                for(i = 0; i < SeedBlockSize - pos2; ++pos) {
                    sSource[pos] = 0;
                    ++i;
                }
            }
            ...
            for(i = 0; i < SeedBlockSize; ++i) {
                String hex = Integer.toHexString(255 & sTarget[i]);
                if (hex.length() == 1) {
                    hex = "0" + hex;
                }
    ...
    public static String decodeString(String inData, byte[] szKey) {
        String target = "";
        int[] sRoundKey = new int[32];
        byte[] temp = new byte[1024];
        SeedEncRoundKey(sRoundKey, szKey);
        byte[] sbuffer = inData.getBytes();
        int rt = sbuffer.length / (SeedBlockSize * 2);
        int p = 0;

        for(int j = 0; j < rt; ++j) {
            int l = 0;
            byte[] sTmpBuf = new byte[BuffSize];
            byte[] sSource = new byte[BuffSize];
            byte[] sTarget = new byte[BuffSize];
            int k = j * SeedBlockSize * 2 + SeedBlockSize * 2;

            int o;
            for(o = j * SeedBlockSize * 2; o < k; ++l) {
                sTmpBuf[l] = sbuffer[o];
                ++o;
            }
            ...
            for(int n = 0; o < 2 * SeedBlockSize; ++n) {
                String hex = (new String(sTmpBuf)).substring(o, o + 2);
    ...
    public static int hexToInt(String sbuffer) {
        int uiNum = 0;
        boolean iConvert = true;
        ...
        for(int i = 0; i < 2; ++i) {
            switch(sbuffer.charAt(i)) {
                ...
                case '8':
                case '9':
                    uiNum = uiNum << 4 | sbuffer.charAt(i) - 48;
                    break;
                case ':':
                case ';':
                case '<':
                ...
                case 'E':
                case 'F':
                    uiNum = uiNum << 4 | sbuffer.charAt(i) - 65 + 10;
                    break;
                case 'a':
                case 'b':
                case 'c':
                ...
                case 'e':
                case 'f':
                    uiNum = uiNum << 4 | sbuffer.charAt(i) - 97 + 10;
            }
        }
